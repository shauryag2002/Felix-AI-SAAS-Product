  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      authorize: async (credentials, req) => {
        // Add your own logic to validate the credentials and return the user object
        // For example, you can check if the email and password match a user in your database
        if (!credentials?.email || !credentials?.password) return null;
        const { email, password } = credentials;
        const yourAuthenticationLogic = async (email: string, password: string) => {
        ...
        if (user) {
          ...
          return {
            ...user,
            id: user.id.toString(), // Ensure the id is of type string
          };
        } else {
